...
デザインがあれば見た目と機能がわかりやすいので、そこから考える。
...
- **「状態管理 + 純粋関数 + 再描画」で考える**
- js ざっくりでいいから 表示（描画 動的に表示する）、イベント、関数を考える
- js 描画は描画専用の関数を実行させてやることにしよう。関数の名前は render としよう。
- js ヘッダーに Date 関数を使って今月の「yyyy 年 m 月」を描画
- js 曜日（DOW）を描画
- js カレンダーセルの中身を描画したいけど、するためには何が必要か？
